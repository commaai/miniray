import os
import sys
import resource
from time import process_time
from pathlib import Path

TASK_CGROUP = Path(os.environ["TASK_CGROUP"]) if "TASK_CGROUP" in os.environ else None

def get_cgroup_cpu_usage(cgroup: Path):
  with (Path('/sys/fs/cgroup') / cgroup / 'cpu.stat').open() as f:
    usage_usec = next(line for line in f if line.startswith("usage_usec")).split()[1]
    cpu_seconds = int(usage_usec) / 1e6
  return cpu_seconds

def get_cgroup_mem_usage(cgroup: Path):
  with (Path('/sys/fs/cgroup') / cgroup / 'memory.peak').open() as f:
    max_bytes = int(f.read())
  return max_bytes

def get_cpu_usage():
  if TASK_CGROUP:
    return get_cgroup_cpu_usage(TASK_CGROUP)
  return process_time()

def get_mem_usage():
  if TASK_CGROUP:
    return get_cgroup_mem_usage(TASK_CGROUP)

  maxrss = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
  if sys.platform != 'linux':
    raise Exception("rusage units unknown")
  return maxrss * 1024 # linux is in KB

def get_gpu_stats(pid, devices):
  import pynvml
  try:
    for device in devices:
      try:
        return pynvml.nvmlDeviceGetAccountingStats(device, pid)
      except pynvml.NVMLError_NotFound:  # ty: ignore[unresolved-attribute]
        continue
  except pynvml.NVMLError:
    pass
  return None

def get_gpu_mem_usage(stats):
  return stats.maxMemoryUsage if stats else 0

def get_gpu_utilization(stats):
  return stats.gpuUtilization / 100 if stats else 0
